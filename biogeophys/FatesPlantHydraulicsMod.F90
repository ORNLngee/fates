module FatesPlantHydraulicsMod
  ...
  use EDTypesMod        , only : SalFileSize           ! Junyan Ding added: salinity data time-length
  ...
  ! Junyan Ding added
  real(r8), parameter :: Eff_rec_soil_layer = 5 ! set the effective soil layer for recruitment
  real(r8) :: Sal2Psi_osm = -0.068  ! covert soil salinity to osmotic potential
  ...
  real(r8), parameter :: max_wb_step_err = 2.e-5_r8   ! original is 1.e-7_r8, Junyan Ding changed to 2.e-5_r8
...
contains
  ...
  subroutine RestartHydrStates(sites,nsites,bc_in,bc_out)
    ...
    do s = 1,nsites
       ...
       if (hlm_use_planthydro_salinity.eq.itrue) then
         if (hlm_model_day >= SalFileSize) then
  ...
  subroutine InitPlantHydStates(site, cohort)
    ...
    !----
    ! Junyan Ding added
    ! initialize cohort-level salinity
    if (hlm_use_planthydro_salinity.eq.itrue) then
  ...
  subroutine UpdatePlantHydrLenVol(ccohort,csite_hydr)
    ...
    real(r8), parameter :: l2sap_vol_donate_frac = 0.5_r8   ! Junyan Ding added
    ...
    ! Junyan Ding: change the following code to calculate the above ground node volume
...
subroutine FuseCohortHydraulics(currentSite,currentCohort, nextCohort, newn)
  ...
  ncohort_hydr => nextCohort%co_hydr
   ...
   ! Junyan Ding added: osmotic potential when calculate btran
...
subroutine HydrSiteColdStart(sites, bc_in )
  ...
  real(r8) :: cur_soil_sal  ! current day soil salinity [PSU] (Junyan Ding)
  integer :: s
  integer :: j,j_t,j_b
  integer :: nsites
  integer :: nlevrhiz
...
subroutine hydraulics_bc ( nsites, sites, bc_in, bc_out, dtime)
  ...
  real(r8) :: soil_Psi_osm        ! soil osmotic potential from soil salinity    [Mpa]
  ...
  do s = 1, nsites
     ...
     soil_Psi_osm = Sal2Psi_osm * cur_soil_sal

     ! Junyan Ding added: remember the past 5 day soil water content for each layer
     ...
     do j = 1,nlevrhiz 
       ...
       if (j==1) then
         if (cur_soil_wtd <= 0) then
            csite_hydr%soil_th_mem(j,1) = bc_in(s)%watsat_sl(j)       ! saturated water content
         else
            csite_hydr%soil_th_mem(j,1) = bc_in(s)%h2o_liqvol_sl(j)   ! volumatric water content
         endif ! j ==1
       else ! j >1
         !  if (cur_soil_wtd <= csite_hydr%zi_rhiz(j-1)) then
         if (cur_soil_wtd <= csite_hydr%zi_rhiz(j)) then              ! try
            csite_hydr%soil_th_mem(j,1) = bc_in(s)%watsat_sl(j)      ! saturated water content
         else
            csite_hydr%soil_th_mem(j,1) = bc_in(s)%h2o_liqvol_sl(j)  ! volumatric water content
         endif
     ...
     if (csite_hydr%current_day < hlm_model_day .and. hlm_use_planthydro_salinity.eq.itrue) then
       ...
       do ft = 1, numpft
         ...
         do j=1,nlevrhiz
            csite_hydr%acc_sal_slpf(j,ft)=max(0._r8,(csite_hydr%acc_sal_slpf(j,ft)+ &
     ...
     do while (associated(cpatch))
        ...
        if(cpatch%nocomp_pft_label.ne.nocomp_bareground)then
           ...
           co_loop1: do while(associated(ccohort))
              ...
              ! Junyan Ding added: reduction of root conductance
              ...
              if (hlm_use_planthydro_salinity.eq.itrue) then
               do j = 1,nlevrhiz
                ...
                do it = 1,soil_th_mem_size
                   ...
                   if  ((csite_hydr%soil_th_mem(j,it)/bc_in(s)%watsat_sl(j)) > 0.9)  then
                ...
                kfr_red_sal(j) = exp(-fr_red_salk*csite_hydr%acc_sal_slpf(j,ft))
              ...
              elseif(hlm_hydr_solver == hydr_solver_1DTaylor ) then
                 ...
                 ! Junyan Ding: added bc_in(s), soikl_Psi_osm and kfr_red(1:nlevrhiz)
...
subroutine ImTaylorSolve1D(slat, slon,recruitflag,csite_hydr,cohort,cohort_hydr,dtime,q_top, &
  ...
  ! Arguments (IN)
  type(bc_in_type),intent(in) :: bc_in
  real(r8), intent(in)                         :: soil_Psi_osm   ! soil osmotic potential [Mpa]
  real(r8), intent(in)                         :: kfr_red(:)     ! fine roots surface conductance reduction ratio
  ...
  real(r8) :: osm_node(n_hypool_tot)          ! osmotic potential on node [Mpa]
  ...
  ! Junyan Ding added: incorporate the root mortality adjusted root conductance
  ...
  loop_root_layers: do jj=1,csite_hydr%nlevrhiz
     ...
     if(do_parallel_stem)then
        ! Junyan Ding noted: this is not right, scale amount of water uptake from a layer
        ...
        ! Junyan Ding added: another formula to calculate the partitioning of root uptake for soil layers
     ...
     ! Junyan Ding: change the following term
     ...
     solution_iteration: do while( .not.solution_found )
        ...
        do istep = 1,nsteps
           ...
           do i = 1,n_hypool_plant
              ...
              h_node(i) =  mpa_per_pa*denh2o*grav_earth*z_node(i) + psi_node(i) + osm_node(i) ! Junyan Ding added osm
           ...
           ! Junyan Ding modified the code below to incorporate osmotic potential in
           ...
           i_up    = j+1
           i_dn    = j
           ! Junyan Ding changed below to add kr_red for both troot and aroot for a given
           ...
           ! Path is between the absorbing root
           ! and the first rhizosphere shell nodes
           ! Junyan Ding added: here to adjust fine root surface conductance
           ...
           ! Junyan Ding added line below to adjust kmax_dn of fine roots
